  ...
  const initNewChatForAgent = async (agent: any, specificAgentId?: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Alex AI: Conversation created in database:", newChatId)
      } catch (error) {
        console.error("❌ Alex AI: Failed to create conversation in database:", error)
      }
    }
  }
    ...
    if (user?.id) {
      try {
        ...
        if (apiMessages && apiMessages.length > 0) {
          ...
          console.log("✅ Alex AI: Loaded", apiMessages.length, "messages from API for chat:", chatId)
        }
      } catch (error) {
        console.error("❌ Alex AI: Failed to load messages from API:", error)
      }
    }
  }
  ...
  const deleteChat = async (chatIdToDelete: string, e?: React.MouseEvent) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Alex AI: Conversation deleted from database:", chatIdToDelete)
      } catch (error) {
        console.error("❌ Alex AI: Failed to delete conversation from database:", error)
      }
    }
  ...
  const confirmRename = async (chatId: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Alex AI: Conversation renamed in database:", chatId, "->", newTitle)
      } catch (error) {
        console.error("❌ Alex AI: Failed to rename conversation in database:", error)
      }
    }
    ...
    if (!currentChatIdForSend) {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Alex AI: New conversation created with messages")
        } catch (error) {
          console.error("❌ Alex AI: Failed to create conversation:", error)
        }
      }
    } else {
      ...
      const typewriterDone = new Promise<void>((resolve) => {
        const tick = () => {
          if (displayedLen < rawText.length) {
            ...
            // stream, yet still animates a couple of characters at a time near the end.
            ...
            >
              ...
              <div className="relative flex items-center justify-between p-3 md:p-4 rounded-xl z-10">
                ...
                <div className="flex items-center gap-2 md:gap-4">
                  ...
                  <a
                    href={`/dashboard/knowledgebase/alex-ai?sharedNamespaceId=${user?.id ?? ""}`}
                ...
                >
                  ...
                  >
                    ...
                    {msg.sender === "ai" && (msg.text === "..." || msg.text === "") ? (
                      <div className="typing-indicator text-sky-500 dark:text-sky-400" aria-label="Alex AI sta scrivendo">
                        <span></span>
                        <span></span>
                        <span></span>
                      </div>
