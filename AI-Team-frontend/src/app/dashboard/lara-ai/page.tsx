...
// --- AGENT ROSTER LIST (FIXED: replaced duplicate lara-ai with alex-ai) ---
const AI_TEAM_LIST = [
  { id: "mike-ai" },
  { id: "tony-ai" },
  { id: "lara-ai" },
  ...
  { id: "niko-ai" },
  { id: "valentina-ai" },
  { id: "alex-ai" },  // FIXED: was "lara-ai" (duplicate)
  { id: "aladino-ai" },
  { id: "jim-ai" },
  { id: "daniele-ai" },
  { id: "dan-ai" },
  ...
  const initNewChatForAgent = async (agent: any, specificAgentId?: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Lara AI: Conversation created in database:", newChatId)
      } catch (error) {
        console.error("❌ Lara AI: Failed to create conversation in database:", error)
      }
    }
  }
    ...
    if (user?.id) {
      try {
        ...
        if (apiMessages && apiMessages.length > 0) {
          ...
          console.log("✅ Lara AI: Loaded", apiMessages.length, "messages from API for chat:", chatId)
        }
      } catch (error) {
        console.error("❌ Lara AI: Failed to load messages from API:", error)
      }
    }
  }
  ...
  const deleteChat = async (chatIdToDelete: string, e?: React.MouseEvent) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Lara AI: Conversation deleted from database:", chatIdToDelete)
      } catch (error) {
        console.error("❌ Lara AI: Failed to delete conversation from database:", error)
      }
    }
  ...
  const confirmRename = async (chatId: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Lara AI: Conversation renamed in database:", chatId, "->", newTitle)
      } catch (error) {
        console.error("❌ Lara AI: Failed to rename conversation in database:", error)
      }
    }
    ...
    if (!currentChatIdForSend) {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Lara AI: New conversation created with messages")
        } catch (error) {
          console.error("❌ Lara AI: Failed to create conversation:", error)
        }
      }
    } else {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Lara AI: User message saved to API")
        } catch (error) {
          console.error("❌ Lara AI: Failed to save user message:", error)
        }
      }
    }
    ...
    try {
      ...
      if (user?.id && currentChatIdForSend) {
        try {
          ...
          console.log("✅ Lara AI: AI message saved to API")
        } catch (error) {
            ...
            >
              ...
              <div className="relative flex items-center justify-between p-3 md:p-4 rounded-xl z-10">
                ...
                <div className="flex items-center gap-2 md:gap-4">
                  ...
                  <a
                    href={`/dashboard/knowledgebase/lara-ai?sharedNamespaceId=${user?.id ?? ""}`}
                  ...
                  {msg.sender === "ai" && (
                    <div className="w-9 h-9 md:w-10 md:h-10 rounded-full shadow-lg shadow-sky-500/30 shrink-0 border-2 border-white dark:border-slate-900 overflow-hidden">
                      <img
                        src={
                          currentAgent.image ||
                          "/assets/agents/Lara-AI-social-strategiest.png"
                        }
                        alt={currentAgent.name}
                        className="w-full h-full object-cover"
                      />
