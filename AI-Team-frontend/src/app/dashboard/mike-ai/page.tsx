  ...
  const initNewChatForAgent = async (agent: any, specificAgentId?: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Mike AI: Conversation created in database:", newChatId)
      } catch (error) {
        console.error("❌ Mike AI: Failed to create conversation in database:", error)
      }
    }
  }
    ...
    if (user?.id) {
      try {
        ...
        if (apiMessages && apiMessages.length > 0) {
          ...
          console.log("✅ Mike AI: Loaded", apiMessages.length, "messages from API for chat:", chatId)
        }
      } catch (error) {
        console.error("❌ Mike AI: Failed to load messages from API:", error)
      }
    }
  }
  ...
  const deleteChat = async (chatIdToDelete: string, e?: React.MouseEvent) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Mike AI: Conversation deleted from database:", chatIdToDelete)
      } catch (error) {
        console.error("❌ Mike AI: Failed to delete conversation from database:", error)
      }
    }
  ...
  const confirmRename = async (chatId: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Mike AI: Conversation renamed in database:", chatId, "->", newTitle)
      } catch (error) {
        console.error("❌ Mike AI: Failed to rename conversation in database:", error)
      }
    }
    ...
    if (!currentChatIdForSend) {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Mike AI: New conversation created with messages")
        } catch (error) {
          console.error("❌ Mike AI: Failed to create conversation:", error)
        }
      }
    } else {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Mike AI: User message saved to API")
        } catch (error) {
          console.error("❌ Mike AI: Failed to save user message:", error)
        }
      }
    }
    ...
    try {
      ...
      if (user?.id && currentChatIdForSend) {
        try {
          ...
          console.log("✅ Mike AI: AI message saved to API")
        } catch (error) {
          console.error("❌ Mike AI: Failed to save AI message:", error)
        }
      }
  ...
  return (
    <>
      ...
      <div className={`flex h-screen w-full bg-tech-grid azure-glow-bg ${isDark ? "dark" : ""}`}>
        ...
        {/* Main Chat Area */}
        <div
          className="flex-1 flex flex-col relative h-full overflow-hidden bg-transparent"
        >
            ...
            >
              ...
              <div className="relative flex items-center justify-between p-2 sm:p-3 md:p-4 rounded-xl z-10">
                ...
                <div className="flex items-center gap-1.5 sm:gap-2 md:gap-4">
                  ...
                  <a
                    href={`/dashboard/knowledgebase/mike-ai?sharedNamespaceId=${user?.id ?? ""}`}
          ...
          <div
            className="sticky bottom-0 px-2 sm:px-4 md:px-8 pb-2 sm:pb-4 md:pb-6"
          >
              ...
              <div className="glass-panel rounded-xl sm:rounded-2xl shadow-xl sm:shadow-2xl border-2 border-sky-200 dark:border-sky-700/50 overflow-hidden">
                <div className="flex items-end gap-2 sm:gap-3 p-2 sm:p-3 md:p-4">
                  ...
                  <textarea
                    ...
                    placeholder={isTokenLimitReached ? `Limite di token raggiunto. Avvia una nuova chat per continuare.` : `Scrivi a ${currentAgent.name}...`}
                    rows={1}
