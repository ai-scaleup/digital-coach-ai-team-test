...
export default function App() {
  ...
  const N8N_ENDPOINT = process.env.NEXT_PUBLIC_VALENTINA_AI_N8N_ENDPOINT || "https://n8n-c2lq.onrender.com/webhook/f5636e0e-1355-439b-b5fd-df0174e3dddb/chat?action=sendMessage"
  ...
  useEffect(() => {
    ...
    const loadConversations = async () => {
      try {
        console.log("📡 Valentina AI: Fetching conversations from API for user:", user.id)
        const conversations = await conversationService.getConversations(user.id, "valentina-ai")
        ...
        if (conversations && conversations.length > 0) {
          ...
          console.log("✅ Valentina AI: Loaded", conversations.length, "conversations from API")
        } else {
          console.log("📝 Valentina AI: No conversations found, creating new chat")
          await initNewChatForAgent(currentAgent)
        }
      } catch (error) {
        console.error("❌ Valentina AI: Failed to load conversations from API:", error)
        await initNewChatForAgent(currentAgent)
      }
    }
  ...
  const initNewChatForAgent = async (agent: any, specificAgentId?: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.error("❌ Valentina AI: Failed to create conversation in database:", error)
      }
    }
  }
    ...
    if (user?.id) {
      try {
        ...
        if (apiMessages && apiMessages.length > 0) {
          ...
          console.log("✅ Valentina AI: Loaded", apiMessages.length, "messages from API for chat:", chatId)
        }
      } catch (error) {
        console.error("❌ Valentina AI: Failed to load messages from API:", error)
      }
    }
  }
  ...
  const deleteChat = async (chatIdToDelete: string, e?: React.MouseEvent) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Valentina AI: Conversation deleted from database:", chatIdToDelete)
      } catch (error) {
        console.error("❌ Valentina AI: Failed to delete conversation from database:", error)
      }
    }
  ...
  const confirmRename = async (chatId: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Valentina AI: Conversation renamed in database:", chatId, "->", newTitle)
      } catch (error) {
        console.error("❌ Valentina AI: Failed to rename conversation in database:", error)
      }
    }
    ...
    if (!currentChatIdForSend) {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Valentina AI: New conversation created with messages")
        } catch (error) {
          console.error("❌ Valentina AI: Failed to create conversation:", error)
        }
      }
    } else {
      ...
      if (user?.id) {
        try {
          ...
          console.error("❌ Valentina AI: Failed to save user message:", error)
        }
      }
    }
    ...
    try {
      ...
      const response = await authenticatedFetch(N8N_ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          metadata: {
            ...
            "agent-memory": `${CURRENT_NAMESPACE.current}-valentina-ai`,
            source: "valentina-ai",
            email: userEmail,
          },
      ...
      if (user?.id && currentChatIdForSend) {
        try {
          ...
          console.log("✅ Valentina AI: AI message saved to API")
        } catch (error) {
