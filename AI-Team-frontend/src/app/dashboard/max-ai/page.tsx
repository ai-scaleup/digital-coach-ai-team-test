...
const AGENTS_DB: Record<string, any> = {
  ...
  "lara-ai": {
    ...
    image: "/assets/agents/Lara-AI.png",
...
export default function App() {
  // --- STATE ---
  const [activeAgentId, setActiveAgentId] = useState<string>("max-ai")
  const currentAgent = AGENTS_DB[activeAgentId] || AGENTS_DB["max-ai"]
  ...
  useEffect(() => {
    ...
    const savedFolders = localStorage.getItem("max-ai-folders")
    if (savedFolders) {
      try {
  ...
  useEffect(() => {
    if (user?.id) {
      ...
      console.log("✅ Max AI: Using user.id for Pinecone namespace:", user.id)
    }
  }, [user?.id])
  ...
  const initNewChatForAgent = async (agent: any, specificAgentId?: string) => {
    ...
    if (agent.name === "Max AI") {
      messageText = `Ciao, sono Max AI, il tuo Business Development Manager virtuale. Sviluppo opportunità di business e partnership strategiche per accelerare la crescita aziendale. Come posso supportarti oggi?`
    }
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Max AI: Conversation created in database:", newChatId)
      } catch (error) {
        console.error("❌ Max AI: Failed to create conversation in database:", error)
      }
    }
  }
    ...
    if (user?.id) {
      try {
        ...
        if (apiMessages && apiMessages.length > 0) {
          ...
          console.log("✅ Max AI: Loaded", apiMessages.length, "messages from API for chat:", chatId)
        }
      } catch (error) {
        console.error("❌ Max AI: Failed to load messages from API:", error)
      }
    }
  }
  ...
  const confirmCreateFolder = () => {
    ...
    localStorage.setItem("max-ai-folders", JSON.stringify(updatedFolders))
  ...
  const deleteFolder = (folderId: string, e: React.MouseEvent) => {
    ...
    localStorage.setItem("max-ai-chats", JSON.stringify(updatedChats))
  ...
  const deleteChat = async (chatIdToDelete: string, e?: React.MouseEvent) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Max AI: Conversation deleted from database:", chatIdToDelete)
      } catch (error) {
        console.error("❌ Max AI: Failed to delete conversation from database:", error)
      }
    }
  ...
  const confirmRename = async (chatId: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Max AI: Conversation renamed in database:", chatId, "->", newTitle)
      } catch (error) {
        console.error("❌ Max AI: Failed to rename conversation in database:", error)
      }
    }
    ...
    if (!currentChatIdForSend) {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Max AI: New conversation created with messages")
        } catch (error) {
          console.error("❌ Max AI: Failed to create conversation:", error)
        }
      }
    } else {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Max AI: User message saved to API")
        } catch (error) {
          console.error("❌ Max AI: Failed to save user message:", error)
        }
      }
    }
    ...
    try {
      ...
      if (user?.id && currentChatIdForSend) {
        try {
          ...
          console.log("✅ Max AI: AI message saved to API")
        } catch (error) {
          console.error("❌ Max AI: Failed to save AI message:", error)
        }
      }
    } catch (error) {
            ...
            <div className="flex items-center justify-between px-3 py-2 rounded-lg bg-slate-50 dark:bg-white/5 border border-sky-100 dark:border-white/5">
              ...
              <button
                onClick={() => {
                  ...
                  localStorage.setItem("max-ai-use-memory", String(!useMemory))
                }}
