...
In alternativa, preferisci una consulenza completa per sviluppare un sales plan strutturato, oppure vuoi concentrarti su una delle 3 aree specifiche sopra menzionate?`
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Tony AI: Conversation created in database:", newChatId)
      } catch (error) {
        console.error("❌ Tony AI: Failed to create conversation in database:", error)
      }
    }
  }
    ...
    if (user?.id) {
      try {
        ...
        if (apiMessages && apiMessages.length > 0) {
          ...
          console.log("✅ Tony AI: Loaded", apiMessages.length, "messages from API for chat:", chatId)
        }
      } catch (error) {
        console.error("❌ Tony AI: Failed to load messages from API:", error)
      }
    }
  }
  ...
  const deleteChat = async (chatIdToDelete: string, e?: React.MouseEvent) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Tony AI: Conversation deleted from database:", chatIdToDelete)
      } catch (error) {
        console.error("❌ Tony AI: Failed to delete conversation from database:", error)
      }
    }
  ...
  const confirmRename = async (chatId: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Tony AI: Conversation renamed in database:", chatId, "->", newTitle)
      } catch (error) {
        console.error("❌ Tony AI: Failed to rename conversation in database:", error)
      }
    }
    ...
    if (!currentChatIdForSend) {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Tony AI: New conversation created with messages")
        } catch (error) {
          console.error("❌ Tony AI: Failed to create conversation:", error)
        }
      }
    } else {
      ...
      if (user?.id) {
        try {
          ...
          console.error("❌ Tony AI: Failed to save user message:", error)
        }
      }
    }
    ...
    try {
      ...
      if (user?.id && currentChatIdForSend) {
        try {
          ...
          console.log("✅ Tony AI: AI message saved to API")
        } catch (error) {
            ...
            >
              ...
              <div className="relative flex items-center justify-between p-3 md:p-4 rounded-xl z-10">
                ...
                <div className="flex items-center gap-2 md:gap-4">
                  ...
                  <a
                    href={`/dashboard/knowledgebase/tony-ai?sharedNamespaceId=${user?.id ?? ""}`}
