...
export default function App() {
  ...
  const N8N_ENDPOINT = process.env.NEXT_PUBLIC_JIM_AI_N8N_ENDPOINT || "https://n8n-c2lq.onrender.com/webhook/bdc4cf07-48f7-4144-ac75-659ab5197b2b/chat?action=sendMessage"

  useEffect(() => {
    if (user?.id) {
      CURRENT_NAMESPACE.current = user.id
      console.log("✅ Jim AI: Using user.id for Pinecone namespace:", user.id)
    }
  }, [user?.id])
  ...
  useEffect(() => {
    ...
    authenticatedFetch(`${API_BASE}/token-usage/${userIdentifier}/JIM`)
  ...
  useEffect(() => {
    ...
    const loadConversations = async () => {
      try {
        console.log("📡 Jim AI: Fetching conversations from API for user:", user.id)
        const conversations = await conversationService.getConversations(user.id, "jim-ai")
        ...
        if (conversations && conversations.length > 0) {
          ...
          console.log("✅ Jim AI: Loaded", conversations.length, "conversations from API")
        } else {
          console.log("📝 Jim AI: No conversations found, creating new chat")
          await initNewChatForAgent(currentAgent)
        }
      } catch (error) {
        console.error("❌ Jim AI: Failed to load conversations from API:", error)
        await initNewChatForAgent(currentAgent)
      }
    }
  ...
  const initNewChatForAgent = async (agent: any, specificAgentId?: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Jim AI: Conversation created in database:", newChatId)
      } catch (error) {
        console.error("❌ Jim AI: Failed to create conversation in database:", error)
      }
    }
  }
    ...
    if (user?.id) {
      try {
        ...
        if (apiMessages && apiMessages.length > 0) {
          ...
          console.log("✅ Jim AI: Loaded", apiMessages.length, "messages from API for chat:", chatId)
        }
      } catch (error) {
        console.error("❌ Jim AI: Failed to load messages from API:", error)
      }
    }
  }
  ...
  const deleteChat = async (chatIdToDelete: string, e?: React.MouseEvent) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Jim AI: Conversation deleted from database:", chatIdToDelete)
      } catch (error) {
        console.error("❌ Jim AI: Failed to delete conversation from database:", error)
      }
    }
  ...
  const confirmRename = async (chatId: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Jim AI: Conversation renamed in database:", chatId, "->", newTitle)
      } catch (error) {
        console.error("❌ Jim AI: Failed to rename conversation in database:", error)
      }
    }
  ...
  const sendMessage = async () => {
    ...
    if (!currentChatIdForSend) {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Jim AI: New conversation created with messages")
        } catch (error) {
          console.error("❌ Jim AI: Failed to create conversation:", error)
        }
      }
    } else {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Jim AI: User message saved to API")
        } catch (error) {
          console.error("❌ Jim AI: Failed to save user message:", error)
        }
      }
    }
    ...
    try {
      ...
      console.log('📤 Jim AI [SEND MESSAGE]: Preparing to send message to N8N...')
      console.log('📤 Jim AI [SEND MESSAGE]: N8N Endpoint:', N8N_ENDPOINT)
      console.log('📤 Jim AI [SEND MESSAGE]: Session ID:', sessionId)
      console.log('📤 Jim AI [SEND MESSAGE]: Chat ID:', currentChatIdForSend)
      ...
      const requestBody = {
        ...
        metadata: {
          ...
          "agent-memory": `${CURRENT_NAMESPACE.current}-jim-ai`,
          source: "jim-ai",
          email: userEmail,
        },
      ...
      console.log('📤 Jim AI [SEND MESSAGE]: Full request body:', JSON.stringify(requestBody, null, 2))
      ...
      console.log('📥 Jim AI [SEND MESSAGE]: Response status:', response.status)
      ...
      if (user?.id && currentChatIdForSend) {
        try {
          ...
          console.log("✅ Jim AI: AI message saved to API")
        } catch (error) {
          console.error("❌ Jim AI: Failed to save AI message:", error)
        }
      }
      ...
      try {
        if (userIdentifier) {
          ...
          await authenticatedFetch(`${API_BASE}/token-usage/${userIdentifier}/JIM/usage`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ totalUsedInputTokens, totalUsedOutputTokens }),
          })

          const updated = await authenticatedFetch(`${API_BASE}/token-usage/${userIdentifier}/JIM`).then((r) =>
            r.ok ? r.json() : null,
          )
      ...
      setIsLoading(false)
      // Input clearing moved to happen immediately after send
    }
  }
            ...
            >
              ...
              <div className="relative flex items-center justify-between p-3 md:p-4 rounded-xl z-10">
                <div className="flex items-center gap-4 md:gap-6">
                  ...
                  <div className="flex items-center gap-4 md:gap-6">
                    <div
                      className="relative w-16 h-16 md:w-20 md:h-20 shrink-0 rounded-full border-[3px] border-sky-400 shadow-[0_0_25px_rgba(56,189,248,0.6)] overflow-hidden bg-slate-950"
                    >
                      <img
                        src={currentAgent.image || "/placeholder.svg"}
                ...
                <div className="flex items-center gap-2 md:gap-4">
                  ...
                  <a
                    href={`/dashboard/knowledgebase/jim-ai?sharedNamespaceId=${user?.id ?? ""}`}
