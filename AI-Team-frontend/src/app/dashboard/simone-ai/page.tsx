...
export default function App() {
  ...
  useEffect(() => {
    ...
    authenticatedFetch(`${API_BASE}/token-usage/${userIdentifier}/SIMONE`)
  ...
  useEffect(() => {
    ...
    const loadConversations = async () => {
      try {
        console.log("📡 Simone AI: Fetching conversations from API for user:", user.id)
        const conversations = await conversationService.getConversations(user.id, "simone-ai")
        ...
        if (conversations && conversations.length > 0) {
          ...
          console.log("✅ Simone AI: Loaded", conversations.length, "conversations from API")
        } else {
          console.log("📝 Simone AI: No conversations found, creating new chat")
          initNewChatForAgent(currentAgent)
        }
      } catch (error) {
        console.error("❌ Simone AI: Failed to load conversations from API:", error)
        initNewChatForAgent(currentAgent)
      }
    }
    ...
    try {
      ...
      try {
        if (userIdentifier) {
          ...
          await authenticatedFetch(`${API_BASE}/token-usage/${userIdentifier}/SIMONE/usage`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ totalUsedInputTokens, totalUsedOutputTokens }),
          })

          const updated = await authenticatedFetch(`${API_BASE}/token-usage/${userIdentifier}/SIMONE`).then((r) =>
            r.ok ? r.json() : null,
          )
            ...
            >
              ...
              <div className="relative flex items-center justify-between p-3 md:p-4 rounded-xl z-10">
                ...
                <div className="flex items-center gap-2 md:gap-4">
                  ...
                  <a
                    href={`/dashboard/knowledgebase/simone-ai?sharedNamespaceId=${user?.id ?? ""}`}
