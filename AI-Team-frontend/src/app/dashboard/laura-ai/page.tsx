...
export default function App() {
    // --- STATE ---
    const [activeAgentId, setActiveAgentId] = useState<string>("laura-ai")
    const currentAgent = AGENTS_DB[activeAgentId] || AGENTS_DB["laura-ai"]
    ...
    useEffect(() => {
        ...
        const savedFolders = localStorage.getItem("laura-ai-folders")
        if (savedFolders) {
            try {
    ...
    const initNewChatForAgent = async (agent: any, specificAgentId?: string) => {
        ...
        if (agent.name === "Laura AI") {
            messageText = `Ciao! Sono Laura AI. Gestisco i tuoi social media, creo calendari editoriali e massimizzo l'engagement. Come posso aiutarti?`
        }
        ...
        if (user?.id) {
            try {
                ...
                console.log("✅ Laura AI: Conversation created in database:", newChatId)
            } catch (error) {
                console.error("❌ Laura AI: Failed to create conversation in database:", error)
            }
        }
    }
        ...
        if (user?.id) {
            try {
                ...
                if (apiMessages && apiMessages.length > 0) {
                    ...
                    console.log("✅ Laura AI: Loaded", apiMessages.length, "messages from API for chat:", chatId)
                }
            } catch (error) {
                console.error("❌ Laura AI: Failed to load messages from API:", error)
            }
        }
    }
    ...
    const confirmCreateFolder = () => {
        ...
        localStorage.setItem("laura-ai-folders", JSON.stringify(updatedFolders))
    ...
    const deleteChat = async (chatIdToDelete: string, e?: React.MouseEvent) => {
        ...
        if (user?.id) {
            try {
                ...
                console.log("✅ Laura AI: Conversation deleted from database:", chatIdToDelete)
            } catch (error) {
                console.error("❌ Laura AI: Failed to delete conversation from database:", error)
            }
        }
    ...
    const confirmRename = async (chatId: string) => {
        ...
        if (user?.id) {
            try {
                ...
                console.log("✅ Laura AI: Conversation renamed in database:", chatId, "->", newTitle)
            } catch (error) {
                console.error("❌ Laura AI: Failed to rename conversation in database:", error)
            }
        }
        ...
        if (!currentChatIdForSend) {
            ...
            if (user?.id) {
                try {
                    ...
                    console.log("✅ Laura AI: New conversation created with messages")
                } catch (error) {
                    console.error("❌ Laura AI: Failed to create conversation:", error)
                }
            }
        } else {
            ...
            if (user?.id) {
                try {
                    ...
                    console.log("✅ Laura AI: User message saved to API")
                } catch (error) {
                    console.error("❌ Laura AI: Failed to save user message:", error)
                }
            }
        }
        ...
        try {
            ...
            if (user?.id && currentChatIdForSend) {
                try {
                    ...
                    console.log("✅ Laura AI: AI message saved to API")
                } catch (error) {
