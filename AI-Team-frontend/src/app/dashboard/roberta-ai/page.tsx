...
export default function App() {
  // --- STATE ---
  const [activeAgentId, setActiveAgentId] = useState<string>("roberta-ai")
  const currentAgent = AGENTS_DB[activeAgentId] || AGENTS_DB["roberta-ai"]
  ...
  const N8N_ENDPOINT = "https://n8n-c2lq.onrender.com/webhook/7e71c393-f23e-4016-a8f1-0e74d76a313d/chat?action=sendMessage"
  ...
  useEffect(() => {
    ...
    const savedFolders = localStorage.getItem("roberta-ai-folders")
    if (savedFolders) {
      try {
    ...
    const loadConversations = async () => {
      try {
        ...
        if (conversations && conversations.length > 0) {
          ...
          console.log("✅ Roberta AI: Loaded", conversations.length, "conversations from API")
        } else {
          console.log("📝 Roberta AI: No conversations found, creating new chat")
          await initNewChatForAgent(currentAgent)
        }
      } catch (error) {
        console.error("❌ Roberta AI: Failed to load conversations from API:", error)
        await initNewChatForAgent(currentAgent)
      }
    }
  ...
  useEffect(() => {
    if (user?.id) {
      ...
      console.log("✅ Roberta AI: Using user.id for Pinecone namespace:", user.id)
    }
  }, [user?.id])
  ...
  const initNewChatForAgent = async (agent: any, specificAgentId?: string) => {
    ...
    if (agent.name === "Roberta AI") {
      messageText = `Ciao, sono Roberta AI. Il tuo SEO Strategist virtuale, progettato per sviluppare una strategia SEO a lungo termine e migliorare la tua presenza online. Come posso supportarti oggi?`
    }
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Roberta AI: Conversation created in database:", newChatId)
      } catch (error) {
        console.error("❌ Roberta AI: Failed to create conversation in database:", error)
      }
    }
  }
    ...
    if (user?.id) {
      try {
        ...
        if (apiMessages && apiMessages.length > 0) {
          ...
          console.log("✅ Roberta AI: Loaded", apiMessages.length, "messages from API for chat:", chatId)
        }
      } catch (error) {
        console.error("❌ Roberta AI: Failed to load messages from API:", error)
      }
    }
  }
  ...
  const confirmCreateFolder = () => {
    ...
    localStorage.setItem("roberta-ai-folders", JSON.stringify(updatedFolders))
  ...
  const deleteFolder = (folderId: string, e: React.MouseEvent) => {
    ...
    localStorage.setItem("roberta-ai-chats", JSON.stringify(updatedChats))
  ...
  const deleteChat = async (chatIdToDelete: string, e?: React.MouseEvent) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Roberta AI: Conversation deleted from database:", chatIdToDelete)
      } catch (error) {
        console.error("❌ Roberta AI: Failed to delete conversation from database:", error)
      }
    }
  ...
  const confirmRename = async (chatId: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Roberta AI: Conversation renamed in database:", chatId, "->", newTitle)
      } catch (error) {
        console.error("❌ Roberta AI: Failed to rename conversation in database:", error)
      }
    }
    ...
    if (!currentChatIdForSend) {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Roberta AI: New conversation created with messages")
        } catch (error) {
          console.error("❌ Roberta AI: Failed to create conversation:", error)
        }
      }
    } else {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Roberta AI: User message saved to API")
        } catch (error) {
          console.error("❌ Roberta AI: Failed to save user message:", error)
        }
      }
    }
    ...
    try {
      ...
      if (user?.id && currentChatIdForSend) {
        try {
          ...
          console.log("✅ Roberta AI: AI message saved to API")
        } catch (error) {
          console.error("❌ Roberta AI: Failed to save AI message:", error)
        }
      }
    } catch (error) {
    ...
    <>
            ...
            <div className="flex items-center justify-between px-3 py-2 rounded-lg bg-slate-50 dark:bg-white/5 border border-sky-100 dark:border-white/5">
              ...
              <button
                onClick={() => {
                  ...
                  localStorage.setItem("roberta-ai-use-memory", String(!useMemory))
                }}
                  ...
                  {msg.sender === "ai" && (
                    <div className="w-9 h-9 md:w-10 md:h-10 rounded-full shadow-lg shadow-sky-500/30 shrink-0 border-2 border-white dark:border-slate-900 overflow-hidden">
                      <img
                        src={
                          currentAgent.image ||
                          "/assets/agents/Valentina-AI-AI-SEO-optimizer.png"
                        }
                        alt={currentAgent.name}
                        className="w-full h-full object-cover"
                      />
      ...
      <PreferencesWizard
        ...
        onComplete={() => {
          ...
          if (userEmail) {
            console.log('🔄 Roberta AI [PREFERENCES]: Reloading preferences...')
