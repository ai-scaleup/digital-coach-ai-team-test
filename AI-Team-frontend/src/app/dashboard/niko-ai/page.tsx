...
const AGENTS_DB: Record<string, any> = {
  ...
  "max-ai": {
    ...
    description: "Sviluppo opportunà di business e partnership strategiche per accelerare la crescita aziendale.",
    primaryColor: "#10b981",
    accentColor: "#34d399",
...
export default function App() {
  ...
  const N8N_ENDPOINT = process.env.NEXT_PUBLIC_NIKO_AI_N8N_ENDPOINT || "https://n8n-c2lq.onrender.com/webhook/b7e1d215-fd19-4404-88ce-b6a8d13db9ad/chat?action=sendMessage"

  useEffect(() => {
    if (user?.id) {
      CURRENT_NAMESPACE.current = user.id
      console.log("✅ Niko AI: Using user.id for Pinecone namespace:", user.id)
    }
  }, [user?.id])
  ...
  useEffect(() => {
    ...
    authenticatedFetch(`${API_BASE}/token-usage/${userIdentifier}/NIKO`)
  ...
  useEffect(() => {
    ...
    const loadConversations = async () => {
      try {
        console.log("📡 Niko AI: Fetching conversations from API for user:", user.id)
        const conversations = await conversationService.getConversations(user.id, "niko-ai")
        ...
        if (conversations && conversations.length > 0) {
          ...
          console.log("✅ Niko AI: Loaded", conversations.length, "conversations from API")
        } else {
          console.log("📝 Niko AI: No conversations found, creating new chat")
          await initNewChatForAgent(currentAgent)
        }
      } catch (error) {
        console.error("❌ Niko AI: Failed to load conversations from API:", error)
        await initNewChatForAgent(currentAgent)
      }
    }
  ...
  const initNewChatForAgent = async (agent: any, specificAgentId?: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Niko AI: Conversation created in database:", newChatId)
      } catch (error) {
        console.error("❌ Niko AI: Failed to create conversation in database:", error)
      }
    }
  }
    ...
    if (user?.id) {
      try {
        ...
        if (apiMessages && apiMessages.length > 0) {
          ...
          console.log("✅ Niko AI: Loaded", apiMessages.length, "messages from API for chat:", chatId)
        }
      } catch (error) {
        console.error("❌ Niko AI: Failed to load messages from API:", error)
      }
    }
  }
  ...
  const deleteChat = async (chatIdToDelete: string, e?: React.MouseEvent) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Niko AI: Conversation deleted from database:", chatIdToDelete)
      } catch (error) {
        console.error("❌ Niko AI: Failed to delete conversation from database:", error)
      }
    }
  ...
  const confirmRename = async (chatId: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Niko AI: Conversation renamed in database:", chatId, "->", newTitle)
      } catch (error) {
        console.error("❌ Niko AI: Failed to rename conversation in database:", error)
      }
    }
    ...
    if (!currentChatIdForSend) {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Niko AI: New conversation created with messages")
        } catch (error) {
          console.error("❌ Niko AI: Failed to create conversation:", error)
        }
      }
    } else {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Niko AI: User message saved to API")
        } catch (error) {
          console.error("❌ Niko AI: Failed to save user message:", error)
        }
      }
    }
    ...
    try {
      ...
      const response = await authenticatedFetch(N8N_ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          metadata: {
            ...
            "agent-memory": `${CURRENT_NAMESPACE.current}-niko-ai`,
            source: "niko-ai",
            email: userEmail,
          },
      ...
      if (user?.id && currentChatIdForSend) {
        try {
          ...
          console.log("✅ Niko AI: AI message saved to API")
        } catch (error) {
          console.error("❌ Niko AI: Failed to save AI message:", error)
        }
      }
      ...
      try {
        if (userIdentifier) {
          ...
          await authenticatedFetch(`${API_BASE}/token-usage/${userIdentifier}/NIKO/usage`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ totalUsedInputTokens, totalUsedOutputTokens }),
          })

          const updated = await authenticatedFetch(`${API_BASE}/token-usage/${userIdentifier}/NIKO`).then((r) =>
            r.ok ? r.json() : null,
          )
          if (updated) { /* token usage UI disabled */ }
        }
      } catch (err) {
        console.error("Niko AI: Failed to update token usage:", err)
      }
    } catch (error) {
            ...
            >
              ...
              <div className="relative flex items-center justify-between p-3 md:p-4 rounded-xl z-10">
                ...
                <div className="flex items-center gap-2 md:gap-4">
                  ...
                  <a
                    href={`/dashboard/knowledgebase/niko-ai?sharedNamespaceId=${user?.id ?? ""}`}
