...
export default function App() {
  ...
  const N8N_ENDPOINT = process.env.NEXT_PUBLIC_DANIELE_AI_N8N_ENDPOINT || "https://n8n-c2lq.onrender.com/webhook/b53858eb-1e73-4798-80ae-13c0d3323f1a/chat?action=sendMessage"

  useEffect(() => {
    if (user?.id) {
      CURRENT_NAMESPACE.current = user.id
      console.log("✅ Daniele AI: Using user.id for Pinecone namespace:", user.id)
    }
  }, [user?.id])
  ...
  useEffect(() => {
    ...
    const loadConversations = async () => {
      try {
        console.log("📡 Daniele AI: Fetching conversations from API for user:", user.id)
        const conversations = await conversationService.getConversations(user.id, "daniele-ai")
        ...
        if (conversations && conversations.length > 0) {
          ...
          console.log("✅ Daniele AI: Loaded", conversations.length, "conversations from API")
        } else {
          console.log("📝 Daniele AI: No conversations found, creating new chat")
          await initNewChatForAgent(currentAgent)
        }
      } catch (error) {
        console.error("❌ Daniele AI: Failed to load conversations from API:", error)
        await initNewChatForAgent(currentAgent)
      }
    }
  ...
  const initNewChatForAgent = async (agent: any, specificAgentId?: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Daniele AI: Conversation created in database:", newChatId)
      } catch (error) {
        console.error("❌ Daniele AI: Failed to create conversation in database:", error)
      }
    }
  }
    ...
    if (user?.id) {
      try {
        ...
        if (apiMessages && apiMessages.length > 0) {
          ...
          console.log("✅ Daniele AI: Loaded", apiMessages.length, "messages from API for chat:", chatId)
        }
      } catch (error) {
        console.error("❌ Daniele AI: Failed to load messages from API:", error)
      }
    }
  }
  ...
  const deleteChat = async (chatIdToDelete: string, e?: React.MouseEvent) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Daniele AI: Conversation deleted from database:", chatIdToDelete)
      } catch (error) {
        console.error("❌ Daniele AI: Failed to delete conversation from database:", error)
      }
    }
  ...
  const confirmRename = async (chatId: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Daniele AI: Conversation renamed in database:", chatId, "->", newTitle)
      } catch (error) {
        console.error("❌ Daniele AI: Failed to rename conversation in database:", error)
      }
    }
    ...
    if (!currentChatIdForSend) {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Daniele AI: New conversation created with messages")
        } catch (error) {
          console.error("❌ Daniele AI: Failed to create conversation:", error)
        }
      }
    } else {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Daniele AI: User message saved to API")
        } catch (error) {
          console.error("❌ Daniele AI: Failed to save user message:", error)
        }
      }
    }
    ...
    try {
      ...
      const response = await authenticatedFetch(N8N_ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          metadata: {
            ...
            "agent-memory": `${CURRENT_NAMESPACE.current}-daniele-ai`,
            source: "daniele-ai",
            email: userEmail,
          },
      ...
      if (user?.id && currentChatIdForSend) {
        try {
          ...
          console.log("✅ Daniele AI: AI message saved to API")
        } catch (error) {
          console.error("❌ Daniele AI: Failed to save AI message:", error)
        }
      }
            ...
            >
              ...
              <div className="relative flex items-center justify-between p-3 md:p-4 rounded-xl z-10">
                ...
                <div className="flex items-center gap-2 md:gap-4">
                  ...
                  <a
                    href={`/dashboard/knowledgebase/daniele-ai?sharedNamespaceId=${user?.id ?? ""}`}
