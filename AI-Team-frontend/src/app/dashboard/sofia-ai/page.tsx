...
export default function App() {
  // --- STATE ---
  const [activeAgentId, setActiveAgentId] = useState<string>("sofia-ai")
  const currentAgent = AGENTS_DB[activeAgentId] || AGENTS_DB["sofia-ai"]
  ...
  const N8N_ENDPOINT = "https://n8n-c2lq.onrender.com/webhook/c84e8999-df92-4730-9e40-1bbfea51a86c/chat?action=sendMessage"
  ...
  useEffect(() => {
    ...
    const savedFolders = localStorage.getItem("sofia-ai-folders")
    if (savedFolders) {
      try {
    ...
    const loadConversations = async () => {
      try {
        ...
        if (conversations && conversations.length > 0) {
          ...
          console.log("✅ Sofia AI: Loaded", conversations.length, "conversations from API")
        } else {
          console.log("📝 Sofia AI: No conversations found, creating new chat")
          await initNewChatForAgent(currentAgent)
        }
      } catch (error) {
        console.error("❌ Sofia AI: Failed to load conversations from API:", error)
        await initNewChatForAgent(currentAgent)
      }
    }
  ...
  const initNewChatForAgent = async (agent: any, specificAgentId?: string) => {
    ...
    if (agent.name === "Sofia AI") {
      messageText = `Ciao, sono Sofia AI. Il tuo SEO Strategist virtuale, progettato per sviluppare una strategia SEO a lungo termine e migliorare la tua presenza online. Come posso supportarti oggi?`
    }
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Sofia AI: Conversation created in database:", newChatId)
      } catch (error) {
        console.error("❌ Sofia AI: Failed to create conversation in database:", error)
      }
    }
  }
    ...
    if (user?.id) {
      try {
        ...
        if (apiMessages && apiMessages.length > 0) {
          ...
          console.log("✅ Sofia AI: Loaded", apiMessages.length, "messages from API for chat:", chatId)
        }
      } catch (error) {
        console.error("❌ Sofia AI: Failed to load messages from API:", error)
      }
    }
  }
  ...
  const confirmCreateFolder = () => {
    ...
    localStorage.setItem("sofia-ai-folders", JSON.stringify(updatedFolders))
  ...
  const deleteFolder = (folderId: string, e: React.MouseEvent) => {
    ...
    localStorage.setItem("sofia-ai-chats", JSON.stringify(updatedChats))
  ...
  const deleteChat = async (chatIdToDelete: string, e?: React.MouseEvent) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Sofia AI: Conversation deleted from database:", chatIdToDelete)
      } catch (error) {
        console.error("❌ Sofia AI: Failed to delete conversation from database:", error)
      }
    }
  ...
  const confirmRename = async (chatId: string) => {
    ...
    if (user?.id) {
      try {
        ...
        console.log("✅ Sofia AI: Conversation renamed in database:", chatId, "->", newTitle)
      } catch (error) {
        console.error("❌ Sofia AI: Failed to rename conversation in database:", error)
      }
    }
    ...
    if (!currentChatIdForSend) {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Sofia AI: New conversation created with messages")
        } catch (error) {
          console.error("❌ Sofia AI: Failed to create conversation:", error)
        }
      }
    } else {
      ...
      if (user?.id) {
        try {
          ...
          console.log("✅ Sofia AI: User message saved to API")
        } catch (error) {
          console.error("❌ Sofia AI: Failed to save user message:", error)
        }
      }
    }
    ...
    try {
      ...
      if (user?.id && currentChatIdForSend) {
        try {
          ...
          console.log("✅ Sofia AI: AI message saved to API")
        } catch (error) {
          console.error("❌ Sofia AI: Failed to save AI message:", error)
        }
      }
    } catch (error) {
            ...
            <div className="flex items-center justify-between px-3 py-2 rounded-lg bg-slate-50 dark:bg-white/5 border border-sky-100 dark:border-white/5">
              ...
              <button
                onClick={() => {
                  ...
                  localStorage.setItem("sofia-ai-use-memory", String(!useMemory))
                }}
