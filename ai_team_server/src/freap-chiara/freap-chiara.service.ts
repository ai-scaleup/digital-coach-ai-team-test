...
@Injectable()
export class FreapChiaraService {
  ...
  private getRelatedSessionWhere(
    sessionId: string,
  ): Prisma.FreapChiaraInboundChatLogWhereInput {
  ...
  async createChatLog(data: Prisma.FreapChiaraInboundChatLogCreateInput) {
    return this.prisma.freapChiaraInboundChatLog.create({
      data,
    });
  }

  async getChatLogsBySessionId(sessionId: string) {
    return this.prisma.freapChiaraInboundChatLog.findMany({
  ...
  async getAllSessions() {
    const sessions = await this.prisma.freapChiaraInboundChatLog.groupBy({
      by: ['sessionId'],
      _max: { createdAt: true },
      _count: { id: true },
